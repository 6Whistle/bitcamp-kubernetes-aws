...
import { findArticlesByBoardId, findArticlesCount } from "@/app/component/articles/service/article-service";
...
export default function ArticleListPage({ params }:any){
  ...
  return (
    ...
    <div className="flex overflow-x-scroll snap-x snap-mandatory max-w-6xl no-scrollbar">
      ...
      >
     <Link href={`${PG.BOARD}${RQ.LIST}`} className="font-medium text-black dark:text-black hover:under">{articles && articles[0]?.boardTitle?.toUpperCase()} Article List</Link>     
      </td>
  </tr>
  <tr>
   ...
   {articles && <DataGrid
    ...
    <MoveButton text={`Write Article`} path={`${PG.ARTICLE}${RQ.SAVE}`}></MoveButton>
    </div>
